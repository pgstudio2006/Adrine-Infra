...
export default function BillingDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return DEMO_STATS.map((s) => ({ ...s, value: "—", trend: "Enable platform runtime" }));
  ...
  const revenueBreakdown = useMemo(() => {
    ...
    return allowDemoFallback() ? DEMO_REVENUE_BREAKDOWN : [];
  }, [useLiveKpis, aggregates.revenueByCategory]);

  const recentTransactions = useMemo(() => {
    if (useLiveKpis && aggregates.recentTransactions.length > 0) {
      return aggregates.recentTransactions;
    }
    return pickPlatformRows(false, [], DEMO_RECENT_TRANSACTIONS);
  }, [useLiveKpis, aggregates.recentTransactions]);
  ...
  return (
    ...
    >
      {!useLiveKpis && !allowDemoFallback() && (
        <p className="text-xs text-muted-foreground -mt-2">
          Enable platform runtime for live billing KPIs.
        </p>
      )}
      {!useLiveKpis && allowDemoFallback() && (
        <p className="text-xs text-muted-foreground -mt-2">
