...
| **Emergency** | Red-flag triage / ER desk registration | `createEmergencyCase` from `/reception/registration` emergency mode → **target:** redirect to `/emergency/triage` with `caseId` + shared `platformOpdVisitId` — see [EMERGENCY_MODULE.md](./EMERGENCY_MODULE.md) §4 |
